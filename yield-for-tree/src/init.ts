...
function yieldForTreeHelper<T extends Instance, Q extends InstanceTree>(object: T, tree: Q): Q | undefined {
	for (const child of object.GetChildren()) {
		...
		if (typeIs(className, "string")) {
			if (child.IsA(className)) {
				tree[childName as keyof Q] = undefined!;
			}
		} else if (className) {
	...
	return Object.isEmpty(tree) ? undefined : tree;
}
